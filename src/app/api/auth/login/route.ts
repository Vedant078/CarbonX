  ...
  try {
    ...
    if (!email) {
      return NextResponse.json(
        { success: false, error: 'MISSING_FIELDS', message: 'Email address is required.' },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      if (!user && (isDemo || cleanedEmail.endsWith('@carbonx.demo') || cleanedEmail.includes('demo'))) {
        await initDatabase();
        user = await queryOne<{ id: string; email: string; password_hash: string }>(
          `SELECT id, email, password_hash FROM users WHERE email = $1`,
          [cleanedEmail]
      ...
      console.warn('[login] PostgreSQL query warning:', dbErr?.message || dbErr);
      dbSuccess = false;
    }
    ...
    if (dbSuccess && user && profile) {
      ...
      if (password && !isDemo) {
        ...
        if (!valid) {
          return NextResponse.json(
            { success: false, error: 'INVALID_CREDENTIALS', message: 'Your email or password is incorrect.' },
            { status: 401 }
          );
        }
      ...
      try {
        ...
        console.warn('[login] Failed to write session to PostgreSQL:', sessErr);
      }
      ...
      // Save to server-side session cache
      db.saveSession(token, userProfile);
    ...
    // --- CASE 2: PostgreSQL DB Error / Fallback Accounts ---
    const demoAccount = INITIAL_USERS.find(
      (u) =>
        u.email.toLowerCase() === cleanedEmail ||
        (cleanedEmail.includes('buyer') && u.role === 'BUYER') ||
        (cleanedEmail.includes('dealer') && u.role === 'DEALER') ||
        (cleanedEmail.includes('logistics') && u.role === 'LOGISTICS')
    );

    if (isDemo || demoAccount) {
      const demoUser = demoAccount || INITIAL_USERS[0];
      const token = `demo-token-${demoUser.role.toLowerCase()}-${Date.now()}`;

      // Save to server-side session cache
      db.saveSession(token, demoUser);
    ...
    return NextResponse.json(
      { success: false, error: 'SERVER_ERROR', message: 'The authentication service is temporarily unavailable.' },
      { status: 500 }
    );
  }
}
